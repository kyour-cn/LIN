<?php
$btitle = "资金记录表";
 $s = $pdo->query("SELECT count(Id) from user_log where uid = '{$_SESSION["user_uid"]}'")->fetch()[0]; 
...
$sql="SELECT * FROM user_log where uid = '{$_SESSION["user_uid"]}' ORDER BY time desc limit {$f[3]},{$f[0]}";
$a = $pdo->query($sql);
            ...
            <tbody>
                <tr>
                    ...
                    if ($rs < 1) {
                        ...
                        foreach ($rows as $r) {
                            echo "<th scope='row'>{$r['Id']}</th><td>{$r['name']}</td><td>{$r['m']}</td><td>{$r['time']}</td></tr>";
                        }
                    }
                    ?>
                </tr>
