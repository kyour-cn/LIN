<?php
if($yms[1]=="add"){
    $gd=array("text"=>"创建工单","uc"=>0);
    $rows=array();
}else{
    $gd = query("SELECT * FROM gd where Id=?", array($yms[1]))->fetch();
    $rows = query("SELECT * FROM gd where gid=? and cid=1 ORDER BY time", array($yms[1]))->fetchAll(PDO::FETCH_ASSOC);
}
?>
<div class="row">
    <div class="col-sm-7 col-xs-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <?php
                    echo '<div class="col-md-9 col-md-offset-3"><div class="alert alert-warning" role="alert"><strong>' . $user['name'] . '：</strong>' . $gd['text'] . '</div></div>';
...
<div class="col-md-7"><div class="alert alert-success" role="alert"><strong>管理员：</strong>哦，这可能是因为您太丑了的原因。</div></div-->
                ...
                <form action="./php/gds.php?upd&<?php echo $yms[1];?>" method="post">
                ...
                <?php }?>
                <a href="view.php?gd" class="btn btn-default pull-right"> 返回列表 </a>
                </form>
                <br>
            </div>
        </div>
